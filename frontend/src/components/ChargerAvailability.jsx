...
const ChargerAvailability = ({ charger, userId, pumpId }) => {
      ...
      try {
        if (selectedSlot) {
          const response = await axios.get(
            `http://localhost:5000/Booking/EV/count?timeSlot=${selectedSlot.value}&pumpId=${pumpId}`
          );
  ...
  const generateTimeSlots = () => {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      const formattedTimeStart = slotTimeStart.toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
        hour12: false
      });
      const formattedTimeEnd = slotTimeEnd.toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
        hour12: false
      });
      slots.push({
        ...
        label: `${formattedTimeStart} - ${formattedTimeEnd}`
      });
    }
    return slots;
  };
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:5000/Booking/EV",
        bookingData
      );
    ...
    const options = {
      ...
      handler: async function (response) {
        ...
        const orderResult = await axios.post(
          "http://localhost:5000/verify-order",
          userData
        );
  ...
  return (
    <div className="ev-charger">
      ...
      {available && (
        <>
          <p>
            Speed: {percent || "0"}% in {time || "0"} Minutes
          </p>
          <p>Slots: {slots}</p>
      ...
      >
        ...
        <form>
          ...
          />
          <button className="btn" type="button" disabled={isSlotDisabled} onClick={() => openRazorpay()}>
            Book
          </button>
        </form>
